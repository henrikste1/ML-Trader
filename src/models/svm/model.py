...
Linear Support Vector Regression (SVML) — regression form.
...
class LinearSVMModel(BaseModel):
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series) -> "LinearSVMModel":
        self._feature_columns = list(X_train.columns)
        self.pipeline = Pipeline([
            ("scaler", MinMaxScaler(feature_range=(-1.0, 1.0))),
            ("svr", SVR(kernel="linear", **self.params)),
        ])
